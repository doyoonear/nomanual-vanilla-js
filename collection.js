...
const appendElementsToGrid = function() {
  const { grid, mainImgContainer, mainImg, mainImgWrapper, leftArrowContainer, rightArrowContainer } = createCollectionElements();
  ...
  const listenSmallImgToShowAsMain = function() {
    for (let i = 0; i < smallImgNodeList.length; i++) {
      ...
      smallImgNodeList[i].addEventListener(
        'click', ()=> showSmallImgAsMainImg({ smallImgDataSrc }), true
      );
    }
  }
